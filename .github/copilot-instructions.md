...
- **Documentation:** All principal documentation should be created in the `docs` folder. Documentation should be concise and clear, avoid unnecsessary verbosity and emojis. For code comments, only add comments that provide additional context or explain non-obvious logic. Avoid commenting on self-explanatory code.
...
### Date Integrity & Actions

- **Server Actions:** Use `use server` for all form submissions and data mutations.
- **Validation:** Use **Zod** for schema validation of all external data (API responses, form inputs).
- **Asynchronous Layout/Page APIs:** Remember that `params` and `searchParams` in Next.js 15+ are Promises and must be awaited.
